...
export default function SwipeRound ({ roundPurpose, mealType, onComplete }: RoundGameProps) {
  const handlePick = () => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Swipe round</Text>
      <Text style={styles.subtitle}>
        {roundPurpose.replace('_', ' & ')} • {mealType}
      </Text>
      <Text style={styles.placeholder}>
        Swipe carousel UI will go here (Tinder-style)
      </Text>
      <TouchableOpacity style={styles.button} onPress={handlePick}>
        <Heart size={24} color="#ffffff" />
        <Text style={styles.buttonText}>Pick & continue</Text>
      </TouchableOpacity>
    </View>
  )
}
