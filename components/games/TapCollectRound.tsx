...
import { MousePointer } from 'lucide-react-native'
...
export default function TapCollectRound ({ roundPurpose, mealType, onComplete }: RoundGameProps) {
  const handlePick = () => {
    if (roundPurpose === 'base') {
      ...
      onComplete({ purpose: 'cooking_method', method: 'steamed' })
    }
  }

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Tap to collect</Text>
      <Text style={styles.subtitle}>
        {roundPurpose.replace('_', ' & ')} • {mealType}
      </Text>
      <Text style={styles.placeholder}>
        Falling items tap-to-collect UI will go here
      </Text>
